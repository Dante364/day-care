<div>
    ...
    <x-main>
        ...
        @php
            $drivers = \App\Models\Children::all(); // Fetch all children

            $headers = [
                ['key' => 'id', 'label' => '#'],
                ['key' => 'name', 'label' => 'Name'],
                ['key' => 'age', 'label' => 'Age'],
                ['key' => 'status', 'label' => 'Attendance Status'], // Column for attendance status
        ...
        <x-table :headers="$headers" :rows="$drivers" striped>
            @foreach($drivers as $driver)
                @php
                    ...
                    $latestAttendance = \App\Models\Attendance::where('child_id', $driver->id)->first();
                    $attendanceStatus = $latestAttendance ? ($latestAttendance->status ? 'Present' : 'Absent') : 'Not Recorded';
                @endphp
                @scope('status', $driver)
                    {{ $attendanceStatus }}
                @endscope
